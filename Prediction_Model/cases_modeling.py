...
class Dataset:
    ...
    def __init__(self, geoids, census_file=None, covid_file=None):
        ...
        self.covid_data = self.covid_data.drop(columns=["deaths"])
...
def plot():
    """ Plot outputs """
    y_test = pd.read_csv("cases.csv", header=0).drop(columns=["geoid"]).to_numpy()
    y_pred = pd.read_csv("cases_predictions.csv", header=0)
